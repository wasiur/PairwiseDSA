...
colnames(base_data) <- c("Time", "reported")

L <- nrow(base_data)
plot(base_data$reported[1:(L-1)])

Tmax <- 80
Tmin <- 0

cond1 <- which(base_data$Time > Tmin)
cond2 <- which(base_data$Time < Tmax) 
cond3 <- which(base_data$Time > Tmin & base_data$Time < Tmax)
...
infection_times <- infection_times[infection_times < Tmax - Tmin]
...
gamma_plot<-ggplot(posterior_samples, aes(x=gamma)) + 
  ...
  theme_classic()
gamma_plot
ggsave("plots/FMD/uninformative/FMD_gamma_histogram_uninformative.pdf", plot=gamma_plot, device="pdf", width = 6, height = 4)
...
tau_plot<-ggplot(posterior_samples, aes(x=tau)) + 
  ...
  theme_classic()
tau_plot
ggsave("plots/FMD/uninformative/FMD_tau_histogram_uninformative.pdf", plot=tau_plot, device="pdf", width = 6, height = 4)
...
rho_plot<-ggplot(posterior_samples, aes(x=rho)) + 
  ...
  theme_classic()
rho_plot
ggsave("plots/FMD/uninformative/FMD_rho_histogram_uninformative.pdf", plot=rho_plot, device="pdf", width = 6, height = 4)
...
write.csv(posterior_samples, file="plots/FMD/uninformative/FMD_DSA_posterior_samples_uninformative.csv")
...
I0_plot<-ggplot(posterior_samples, aes(x=I0)) + 
  ...
  theme_classic()
I0_plot
ggsave("plots/FMD/uninformative/FMD_I0_histogram_uninformative.pdf", plot=I0_plot, device="pdf", width = 6, height = 4)
...
Eff_popsize_plot<-ggplot(posterior_samples, aes(x=effective_population_size)) + 
  ...
  theme_classic()
Eff_popsize_plot
ggsave("plots/FMD/uninformative/FMD_Effective_popsize_histogram_uninformative.pdf", plot=Eff_popsize_plot, device="pdf", width = 6, height = 4)
...
Total_popsize_plot<-ggplot(posterior_samples, aes(x=total_population_size)) + 
  ...
  theme_classic()
Total_popsize_plot
ggsave("plots/FMD/uninformative/FMD_Total_popsize_histogram_uninformative.pdf", plot=Total_popsize_plot, device="pdf", width = 6, height = 4)
